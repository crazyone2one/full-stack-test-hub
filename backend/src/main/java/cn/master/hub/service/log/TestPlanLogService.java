...
import cn.master.hub.dto.LogInsertModule;
import cn.master.hub.dto.request.TestPlanBatchEditRequest;
...
public class TestPlanLogService {
    ...
    public LogDTO scheduleLog(String id) {
        ...
        SystemProject project = projectMapper.selectOneById(testPlan.getProjectId());
    ...
    public void saveUpdateLog(TestPlan oldTestPlan, TestPlan newTestPlan, String projectId, String operator, String requestUrl, String requestMethod) {
        ...
        LogDTO dto = LogDTOBuilder.builder()
                ...
                .originalValue(JacksonUtils.toJSONBytes(oldTestPlan))
                .modifiedValue(JacksonUtils.toJSONBytes(newTestPlan))
                .createUser(operator)
                .build().getLogDTO();
        operationLogService.add(dto);
    }
    ...
    public void saveDeleteLog(TestPlan deleteTestPlan, String operator, String requestUrl, String requestMethod) {
        SystemProject project = projectMapper.selectOneById(deleteTestPlan.getProjectId());
        LogDTO dto = LogDTOBuilder.builder()
                ...
                .originalValue(JacksonUtils.toJSONBytes(deleteTestPlan))
                .createUser(operator)
                .build().getLogDTO();
        operationLogService.add(dto);
    }
    ...
    public LogDTO archivedLog(String id) {
        ...
        dto.setOriginalValue(JacksonUtils.toJSONBytes(testPlan));
        return dto;
    }
    ...
    public void saveBatchLog(List<TestPlan> plans, String operator, String requestUrl, String requestMethod, String requestType) {
        ...
        SystemProject project = projectMapper.selectOneById(plans.getFirst().getProjectId());
        ...
        for (TestPlan plan : plans) {
            LogDTO dto = LogDTOBuilder.builder()
                    ...
                    .originalValue(JacksonUtils.toJSONBytes(plan))
                    .createUser(operator)
                    .build().getLogDTO();
            list.add(dto);
        }
    ...
    public List<LogDTO> batchEditLog(TestPlanBatchEditRequest request) {
        ...
        if (CollectionUtils.isNotEmpty(ids)) {
            List<TestPlan> testPlans = QueryChain.of(TestPlan.class).where(TestPlan::getId).in(ids)
                    .and(TestPlan::getProjectId).eq(request.getProjectId()).list();
    ...
    private String getLogModule(TestPlan testPlan) {
        if (Strings.CS.equals(testPlan.getType(), TestPlanConstants.TEST_PLAN_TYPE_PLAN)) {
